...
class Comb(nn.Module):
    def __init__(self, extractor, in_size, class_num) -> None:
        super().__init__()
        self.backbone = copy.deepcopy(extractor)

        self.fc = nn.Sequential(
            nn.BatchNorm1d(in_size),
            nn.Linear(in_size, class_num)
        )
    ...
    def disable_bn(self):
        self.backbone.eval()
        for layer in self.backbone.named_modules():
...
def train(extractor, rot_num, in_size, epochs, dataloader, device):
    ...
    # optimizer = SGD(net.fc.parameters(), lr=1e-2,
    #                     momentum=0.9, weight_decay=5e-4)
    optimizer = SGD(net.fc.parameters(), lr=1e-1,
                        momentum=0.9, weight_decay=5e-4)
    net.train()
